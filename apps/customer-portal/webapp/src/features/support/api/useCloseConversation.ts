...
/**
 * Closes a Novera conversation via POST /conversations/:id/close, moving it to
 * the terminal Close state so it can no longer be resumed. Used when a user
 * explicitly closes a chat from the chat list. On success, refreshes the
 * conversation list and stats for the project.
 *
  ...
  return useMutation<void, Error, string>({
    mutationFn: async (conversationId: string): Promise<void> => {
      ...
      const response = await authFetch(
        `${baseUrl}/conversations/${conversationId}/close`,
        { method: "POST" },
      );
      if (!response.ok) {
      ...
      logger.debug("[useCloseConversation] Conversation closed:", {
        conversationId,
      });
    },
    onSuccess: () => {
