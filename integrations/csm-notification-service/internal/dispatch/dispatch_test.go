...
func TestDispatcher_Handle_CaseMentioned(t *testing.T) {
	...
	record := eventbus.Record{Value: []byte(`{"type":"case.mentioned","entityId":"CASE-1","payload":{"mentionerName":"Commenter","projectId":"PROJ-1","caseId":"CASE-1","caseTitle":"Something broke","caseComment":"see this please","commentId":"C-1","recipients":["test-recipient@example.com"]}}`)}
	...
	if !strings.Contains(mock.calls[0].htmlBody, "see this please") {
		t.Error("htmlBody does not contain the comment text")
	}
	if !strings.Contains(mock.calls[0].htmlBody, "mentioned you") {
		t.Error("htmlBody does not use the mention wording")
	}
}

// TestDispatcher_Handle_CaseMentioned_InternalNote_UsesInternalMentionLayout
// verifies that isInternalNote:true routes through RenderInternalMentionEmail
// instead of RenderMentionEmail: no "Re: <title>" strap, and wso2CaseId (not
// caseNumber) as the case reference — mirrors
// TestDispatcher_Handle_CommentAdded_InternalNote_UsesInternalNoteLayout.
func TestDispatcher_Handle_CaseMentioned_InternalNote_UsesInternalMentionLayout(t *testing.T) {
	mock := &mockEmailSender{}
	d := newTestDispatcher(mock, &mockGoogleChatSender{}, &mockCallSender{})

	record := eventbus.Record{Value: []byte(`{"type":"case.mentioned","entityId":"CASE-1","payload":{"mentionerName":"Agent","projectId":"PROJ-1","caseId":"CASE-1","caseNumber":"CS0001001","wso2CaseId":"WSO2-1000","caseTitle":"Something broke","caseComment":"internal only","commentId":"C-1","isInternalNote":true,"recipients":["agent@wso2.com"]}}`)}
	...
	if strings.Contains(body, "Re: Something broke") {
		t.Error("htmlBody carries the regular layout's \"Re: <title>\" strap, which the internal-mention layout must not have")
	}
	if !strings.Contains(body, "WSO2-1000") {
		t.Error("htmlBody does not use wso2CaseId as the case reference")
	}
...
func TestDispatcher_Handle_CaseMentioned_LinksToCommentFragment(t *testing.T) {
	...
	record := eventbus.Record{Value: []byte(`{"type":"case.mentioned","entityId":"CASE-1","payload":{"mentionerName":"Commenter","projectId":"PROJ-1","caseId":"CASE-1","caseTitle":"Something broke","caseComment":"fixed it","commentId":"C-1","recipients":["test-recipient@example.com"]}}`)}
...
// — same per-group idempotency tracking, different event type.
func TestDispatcher_Handle_CaseMentioned_RetryDoesNotResendSucceededGroup(t *testing.T) {
	...
	record := eventbus.Record{Topic: "case-events", Partition: 1, Offset: 42, Value: []byte(`{"type":"case.mentioned","entityId":"CASE-1","payload":{"mentionerName":"Commenter","projectId":"PROJ-1","caseId":"CASE-1","caseTitle":"Something broke","caseComment":"fixed it","commentId":"C-1","recipients":["customer@acme.com","agent@wso2.com"]}}`)}
