...
//     poll — no-op.
//   - Current tier is ABOVE the stored cursor: alert for every checkpoint
//     strictly between the stored cursor and the current tier, in
//     ascending order, advancing the stored cursor after each one succeeds
//     — so a failure partway through (e.g. Chat is down) still keeps
//     whatever alerted successfully and retries only the remainder on the
//     next Tick, rather than losing progress or double-alerting an already
//     -sent tier.
//
...
// breach card for one newly-crossed tier. Both must succeed for
// processStatus to advance the cursor past this tier — a failure in either
// leaves the cursor where it was, so the next Tick retries this exact tier
