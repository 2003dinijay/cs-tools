...
Contacts sub-resource, plus a subset of Case operations, plus incident creation
and search, to third-party (M2M) consumers. It forwards requests to the entity
service and returns responses as-is — it does not shape or authenticate on
...
|---------|----------|-------|
| `entity` | Entity service | Account/Project + Contacts sub-resource, Case (patch + comment create), Opportunity/Invoice/ProjectOpportunityLink (read-only), incident creation/search; raw `[]byte` passthrough |
