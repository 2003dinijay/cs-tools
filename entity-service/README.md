...
| `AUTH_ISSUER` / `AUTH_JWKS_URL` | Asgardeo issuer and JWKS URL for validating the Asgardeo tokens (`x-user-id-token` user ID token, `x-jwt-assertion` client-credentials token) -- always on, there is no flag to disable it. Required; the JWKS must load at startup or the process exits. A present-but-invalid token is a 401 on every route |
