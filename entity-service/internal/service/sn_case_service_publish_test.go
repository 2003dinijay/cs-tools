...
// newTestMentionClient extends newTestCommentClient with a stub for the
// POST /users/search call resolveMentionedUserEmails issues to resolve
// CreateCaseCommentRequest.MentionedUserIDs.
func newTestMentionClient(t *testing.T, getCaseBody, createCommentBody, searchCommentsBody, usersSearchBody string) *integrationservice.Client {
	t.Helper()
	return newTestCaseClient(t, func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		switch {
		...
		case r.Method == http.MethodPost && r.URL.Path == "/comments/search":
			_, _ = w.Write([]byte(searchCommentsBody))
		case r.Method == http.MethodPost && r.URL.Path == "/users/search":
			_, _ = w.Write([]byte(usersSearchBody))
		case strings.HasSuffix(r.URL.Path, "/tags"):
			_, _ = w.Write([]byte(`{"tags":[]}`))
		default:
			_, _ = w.Write([]byte(getCaseBody))
...
// TestSNCaseService_CreateCaseComment_PublishesCaseMentioned verifies the
// happy path: after a successful comment create with MentionedUserIDs set,
// publishCaseMentioned resolves each mentioned id to an email via
// resolveMentionedUserEmails's /users/search lookup and publishes
// case.mentioned with the comment author's name (reused from the same
// resolveCommentAuthorName lookup case.comment_added's own test exercises,
// not re-fetched — see publishCommentAdded's own doc comment) and the
// mentioned users as Recipients. The case's watch list is empty here so
// case.comment_added itself is skipped (see
// TestSNCaseService_CreateCaseComment_SkipsPublishWhenNoWatchers), isolating
// this test to case.mentioned's own publish call.
func TestSNCaseService_CreateCaseComment_PublishesCaseMentioned(t *testing.T) {
	caseSysid := sysid32('a')
	projectSysid := sysid32('b')
	commentSysid := sysid32('d')
	mentionedSysid := sysid32('e')
	caseID := sysidToUUID(caseSysid)
	commentID := sysidToUUID(commentSysid)
	mentionedUserID := sysidToUUID(mentionedSysid)

	getCaseBody := `{
		"id": "` + caseSysid + `",
		"internalId": "WSO2-040",
		"number": "CS0040001",
		"title": "Mentioning a teammate",
		"description": "d",
		"createdOn": "2026-01-02 10:00:00",
		"createdBy": "jane.doe@example.com",
		"createdByFullName": "Jane Doe",
	...
	searchCommentsBody := `{
		"comments": [
			{"id": "` + commentSysid + `", "referenceId": "` + caseSysid + `", "content": "cc @mentioned.user", "type": "comments", "createdOn": "2026-01-02 11:00:00", "createdBy": "agent.smith", "createdByFullName": "Agent Smith"}
		],
		"offset": 0, "limit": 20, "totalRecords": 1
	}`
	usersSearchBody := `{
		"users": [
			{"id": "` + mentionedSysid + `", "userName": "muser", "name": "Mentioned User", "email": "mentioned.user@wso2.com", "userType": "internal", "active": true}
		],
		"offset": 0, "limit": 1, "totalRecords": 1
	}`

	client := newTestMentionClient(t, getCaseBody, createCommentBody, searchCommentsBody, usersSearchBody)
	publisher := &mockEventPublisher{}
	svc := NewServiceNowCaseService(client, nil, publisher)
	...
	req := domain.CreateCaseCommentRequest{
		CaseID:           caseID,
		Type:             domain.CommentTypeComment,
		Content:          "cc @mentioned.user",
		MentionedUserIDs: []string{mentionedUserID},
	}
	...
	call := publisher.calls[0]
	if call.eventType != events.TypeCaseMentioned {
		t.Errorf("eventType = %q, want %q", call.eventType, events.TypeCaseMentioned)
	}
	if call.entityID != caseID {
		t.Errorf("entityID = %q, want %q", call.entityID, caseID)
	}
	...
	if payload.MentionerName != "Agent Smith" {
		t.Errorf("mentionerName = %q, want %q", payload.MentionerName, "Agent Smith")
	}
	wantProjectID := sysidToUUID(projectSysid)
	if payload.ProjectID != wantProjectID {
		t.Errorf("projectId = %q, want %q", payload.ProjectID, wantProjectID)
	...
	if len(payload.Recipients) != 1 || payload.Recipients[0] != "mentioned.user@wso2.com" {
		t.Errorf("recipients = %v, want [mentioned.user@wso2.com]", payload.Recipients)
	}
}

// TestSNCaseService_CreateCaseComment_SkipsPublishCaseMentionedWhenUnresolved
// verifies that publishCaseMentioned skips publishing (rather than sending
// an event with an empty Recipients list, which events.Validate would
// reject anyway) when none of MentionedUserIDs resolves to a ServiceNow
// user — e.g. a stale id for a deactivated or deleted user — and that this
// never fails the comment write itself.
func TestSNCaseService_CreateCaseComment_SkipsPublishCaseMentionedWhenUnresolved(t *testing.T) {
	caseSysid := sysid32('a')
	projectSysid := sysid32('b')
	commentSysid := sysid32('d')
	mentionedSysid := sysid32('e')
	...
	searchCommentsBody := `{
		"comments": [
			{"id": "` + commentSysid + `", "referenceId": "` + caseSysid + `", "content": "cc @gone.user", "type": "comments", "createdOn": "2026-01-02 11:00:00", "createdBy": "agent.smith", "createdByFullName": "Agent Smith"}
		],
		"offset": 0, "limit": 20, "totalRecords": 1
	}`
	// No matching user comes back for the mentioned id.
	usersSearchBody := `{"users":[],"offset":0,"limit":1,"totalRecords":0}`
	...
	if len(publisher.calls) != 0 {
		t.Fatalf("expected no publish call when no mentioned user resolves, got %d", len(publisher.calls))
	}
}
